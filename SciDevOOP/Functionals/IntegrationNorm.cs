...
class IntegrationNorm : IFunctional
{
    ...
    {
        var integral = SimpsonIntegration(function, LowerBound, UpperBound, NumberOfPoints);
    ...
    private double SimpsonIntegration(IFunction function, double a, double b, int n)
    {
        if (n % 2 == 1) n++;
        var h = (b - a) / n;
        var sum = SquareError(function, a) + SquareError(function, b);
        for (var i = 1; i < n; i++)
        {
            var x = a + i * h;
            var value = SquareError(function, x);
            if (i % 2 == 0) sum += 2 * value;
            else sum += 4 * value;
        }
        return sum * h / 3;
    }
    ...
    {
        // Find the nearest point for y(x)
        var targetY = InterpolateY(x);
        var functionValue = function.Value(new Vector() { x });
        var error = functionValue - targetY;
        return error * error;
    }
