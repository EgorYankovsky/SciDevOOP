...
class ThinPlateSpline : IParametricFunction
{
    ...
    class InternalThinPlateSpline : IFunction
    {
        ...
        public static InternalThinPlateSpline Create(int PointsAmount, IVector points)
        {
            ...
            Task matrixGeneration = Task.Run(() => GenerateMatrix(A, PointsAmount, dimension, points));
            Task vectorGeneration = Task.Run(() => GenerateVector(b, PointsAmount, dimension, points));
            Task.WaitAll(matrixGeneration, vectorGeneration);
        ...
        private static void GenerateMatrix(Matrix A, int PointsAmount, int dimension, IVector points)
        {
            
            for (int i = 0; i < PointsAmount + dimension; ++i)
            {
                A.Add([]);
                var pnt1 = new Vector(points.Skip(i * dimension).Take(dimension).SkipLast(1));
                for (int j = 0; j < PointsAmount + dimension; ++j)
                {
                    ...
                    else 
                    {
                        ...
                        {
                            var pnt2 = new Vector(points.Skip(j * dimension).Take(dimension).SkipLast(1));
        ...
        public static void GenerateVector(IVector b, int PointsAmount, int dimension, IVector points)
        {
            for (int i = 0; i < PointsAmount + dimension; ++i)
            {
        ...
        public double Value(IVector point)
        {
            ...
            if (Points.Count / PointsAmount != point.Count + 1) throw new ArgumentException("Point dimension isn't equal to spline dimension.");
            ...
            var dimension = point.Count + 1;

            for (int i = 0; i < point.Count; ++i) ans += point[i] * a[i + 1];

            for (int j = 0; j < PointsAmount; ++j) 
                ans += w[j] * Phi(Distance(point, new Vector(Points.Skip(j * dimension).Take(point.Count))));
